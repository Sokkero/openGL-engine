...
class DebugUtils
{
    public:
         ...
         */
        static void PrintHumanReadableTimeDuration(
                double time,
                const std::string& text = "",
                const bool includeNanoSeconds = false
        )
        {
            ...
            if(includeNanoSeconds)
            {
                ...
                std::cout << text << hours << "h " << minutes << "m " << seconds << "s " << milliseconds
                          << "ms " << nanoseconds << "ns" << std::endl;
            }
            else
            {
                std::cout << text << hours << "h " << minutes << "m " << seconds << "s " << milliseconds
